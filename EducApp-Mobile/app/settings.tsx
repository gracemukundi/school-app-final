  ...
  useEffect(() => {
    const getDetails = async () => {
      try {
        ...
        // 🔥 CORRECTION DE L'URL : On teste avec 'schools' au pluriel 
        // car c'est la norme souvent utilisée dans tes autres fichiers routes
        const res = await axios.get(`${API_URL}/schools/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        
        setSchool(res.data);
      } catch (err: any) {
        console.error("Erreur détails école:", err.response?.status);
        
        // Si le pluriel 'schools' échoue aussi (404), on tente le singulier par sécurité
        if (err.response?.status === 404) {
             try {
                const token = await AsyncStorage.getItem('userToken');
                const resAlt = await axios.get(`${API_URL}/school/${id}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                setSchool(resAlt.data);
                return;
             } catch (e) {
                Alert.alert("Erreur 404", "La route API est introuvable sur le serveur.");
             }
        }
        Alert.alert("Erreur", "Impossible de charger les détails de l'école");
      } finally {
        setLoading(false);
      }
    };
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20 }}>
        <View style={styles.mainCard}>
          ...
          <Text style={styles.schoolName}>{school?.nom || "École inconnue"}</Text>
        ...
        <TouchableOpacity 
          ...
          onPress={() => router.push({ pathname: '/(tabs)/licences', params: { schoolId: id as string } } as any)}
        >
...
function InfoRow({ icon, label, value, color = '#64748b' }: any) {
  return (
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: '#fff', flexDirection: 'row', alignItems: 'center', elevation: 2,
  },
  ...
  mainCard: { backgroundColor: '#fff', borderRadius: 25, padding: 30, alignItems: 'center', elevation: 4, marginBottom: 20 },
  ...
  actionBtn: { backgroundColor: '#1e3a8a', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: 18, borderRadius: 20, marginTop: 25, elevation: 3 },
